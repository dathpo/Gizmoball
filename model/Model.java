...
public class Model extends Observable implements IModel {
	...
	private boolean cSquare, cCircle, cTriangle, cAbsorber = false;
	...
	public void userPlacedBumper(double x, double y){
		if(cSquare || cCircle || cTriangle){
			if(cCircle){
				bumpers.add(new CircleBumper(x, y, Color.black));
			}else if(cSquare){
				bumpers.add(new SquareBumper(x, y, Color.black));
			}else if(cTriangle){
				bumpers.add(new TriangleBumper(x, y, Color.black));
			}
	...
	public void setCurrentBumper(int x){
		switch(x){
			case 0:
				cSquare = true;
				cCircle = false;
		...
		System.out.println("Square: " + cSquare + " Circle: " + cCircle + " Triangle: " + cTriangle);
		...
		if(cAbsorber){
			for(int x = (int)x1; x <= x2; x++){
				for(int y = (int)y1; y <= y2; y++){
					absorbers.add(new Absorber(x, y, Color.MAGENTA));
